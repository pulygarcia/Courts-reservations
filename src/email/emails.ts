...
type FixedReservationEmailData = {
  name: string;
  courtId: number;
  dayOfWeek: string;
  startTime: string;
...
export async function sendReservationCancellationNotification({
  court,
  date,
  startTime,
  endTime,
  ...
  <div style="font-family: Arial, sans-serif; background-color:#f9fafb; padding:20px;">
    <div style="max-width:600px; margin:0 auto; background:white; border-radius:12px; box-shadow:0 2px 6px rgba(0,0,0,0.1); overflow:hidden;">
      ...
      <div style="padding:24px;">
        <p style="font-size:16px; color:#111827;">
          Se ha cancelado una reserva en <strong>El Galpón Pádel</strong>.
        </p>

        <table style="width:100%; border-collapse:collapse; margin-top:16px;">
          <tr>
            <td style="padding:8px; border:1px solid #e5e7eb;">${court}</td>
          </tr>
          <tr>
            <td style="padding:8px; border:1px solid #e5e7eb; font-weight:bold;">Fecha</td>
            <td style="padding:8px; border:1px solid #e5e7eb;">${date}</td>
  ...
  await transporter.sendMail({
    ...
    text: `Se canceló la reserva en ${court} el ${date} de ${startTime} a ${endTime}`,
    html,
  });
}

export async function sendFixedReservationNotification({
  name,
  courtId,
  dayOfWeek,
  startTime,
  endTime,
}: FixedReservationEmailData) {
  ...
  <div style="font-family: Arial, sans-serif; background-color:#f9fafb; padding:20px;">
    <div style="max-width:600px; margin:0 auto; background:white; border-radius:12px; box-shadow:0 2px 6px rgba(0,0,0,0.1); overflow:hidden;">
      ...
      <div style="background-color:#2563eb; padding:16px; text-align:center;">
        <h1 style="color:white; margin:0;">Nueva Reserva Fija 📅</h1>
      </div>

      <div style="padding:24px;">
        <p style="font-size:16px; color:#111827;">
          Se ha registrado una nueva reserva fija en <strong>El Galpón Pádel</strong>.
        </p>
        ...
        <table style="width:100%; border-collapse:collapse; margin-top:16px;">
          ...
          <tr>
            <td style="padding:8px; border:1px solid #e5e7eb; font-weight:bold;">Día</td>
            <td style="padding:8px; border:1px solid #e5e7eb;">${dayOfWeek}</td>
          </tr>
          <tr>
            <td style="padding:8px; border:1px solid #e5e7eb; font-weight:bold;">Hora</td>
            <td style="padding:8px; border:1px solid #e5e7eb;">${startTime} - ${endTime}</td>
  ...
  await transporter.sendMail({
    from: 'reservas-elgalpon@gmail.com',
    to: 'galponpadelclub@gmail.com',
    subject: "Nueva reserva FIJA registrada ✔",
    text: `Nueva reserva fija de ${name} en cancha ${courtId} los ${dayOfWeek} de ${startTime} a ${endTime}`,
    html,
  });
}

export async function sendFixedReservationCancelledNotification({
  court,
  dayOfWeek,
  startTime,
  endTime,
}: CancelFixedReservation) {
  const html = `
  <div style="font-family: Arial, sans-serif; background-color:#f9fafb; padding:20px;">
    <div style="max-width:600px; margin:0 auto; background:white; border-radius:12px; box-shadow:0 2px 6px rgba(0,0,0,0.1); overflow:hidden;">
      
      <div style="background-color:#dc2626; padding:16px; text-align:center;">
        <h1 style="color:white; margin:0;">Reserva Fija Cancelada ❌</h1>
      </div>

      <div style="padding:24px;">
        <p style="font-size:16px; color:#111827;">
          Se ha cancelado una reserva fija en <strong>El Galpón Pádel</strong>.
        </p>
  ...
  await transporter.sendMail({
    from: "reservas-elgalpon@gmail.com",
    to: "galponpadelclub@gmail.com",
    subject: "Reserva FIJA cancelada ❌",
    text: `Se canceló la reserva fija en ${court} los ${dayOfWeek} de ${startTime} a ${endTime}`,
    html,
  });
}